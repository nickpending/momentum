...
- `{CORRELATION_ID}` — Unique ID for correlating this agent with parent Task
...
**First action after reading this file** — emit activation event for Argus tracking.
...
argus-send --source momentum --type agent \
  ...
  --status activated \
  --message "Agent {subagent_type} started" \
  --data '{"correlation_id": "{CORRELATION_ID}", "subagent_type": "{subagent_type}"}'
```

Replace `{subagent_type}` with your agent type (e.g., task-planner, code-reviewer, Explore).

The `correlation_id` enables exact-match correlation with the parent Task.
