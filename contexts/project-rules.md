...
- NEVER mention MVPs or shortcuts — focus on long-term solutions that are maintainable, matched to the problem domain and not just quick wins.
- NEVER hardcode secrets or configuration values (prefer config files or env vars).
- Never commit .env files, credentials, or any .workflow/ state (sensitive data risk).
...
- ALWAYS resume agents when gaps arise from their work or the agents have questions.
