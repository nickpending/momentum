...
/**
 * Momentum Stop Hook
 * Processes Voice: summaries and speaks them via lspeak TTS
 */
...
async function extractLastAssistantMessage(transcriptPath: string): Promise<string | null> {
  try {
    ...
    debugLog('StopHook', 'Parsed JSONL entries', { entryCount: jsonlEntries.length });
...
function splitIntoSentences(text: string): string[] {
  ...
  try {
    ...
    debugLog('StopHook', 'Sentence segmentation failed, using fallback', { error: String(error) });
    return [text];
  }
}
 ...
 */
function shouldUseCache(mode: string, config: ReturnType<typeof loadConfig>): boolean {
  // Get verbosity level for current mode
  const verbosity = config.voice.verbosity[mode as keyof typeof config.voice.verbosity] || 'normal';
...
async function main() {
  try {
    ...
    const lastMessageContent = await extractLastAssistantMessage(data.transcript_path);
    ...
    debugLog('StopHook', `Split into ${sentences.length} sentences`, { sentences });
